...
func body(data: Dictionary):
	...
	self.model.extend(self.model.near_stop_deceleration_cm).call(params, result)
	var msg = "thr=" + str(params.throttle) + " brk=" + str(params.brake) + " gear=" + str(params.gear) + " v=" + str(params.linear_velocity)
	assert_almost_eq(params.linear_velocity, expected_linear, EPSILON * Vector3.ONE, str(current_line) + " vel " + msg)
	assert_almost_eq(params.angular_velocity, expected_angular, EPSILON * Vector3.ONE, str(current_line) + " ang " + msg)
